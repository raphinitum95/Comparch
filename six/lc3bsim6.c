...
/************************* MEM_stage() *************************/
int MEM_PCMUX;
int TRAP_PC, TARGET_PC, branch;
int v_mem_ld_reg, v_mem_ld_cc, mem_reg_id;
void MEM_stage() {
    ...
    z = (PS.MEM_IR >> 10) & 0x01;
    p = (PS.MEM_IR >> 9) & 0x01;
    MEM_PCMUX = PS.MEM_V ? Get_TRAP_OP(PS.MEM_CS) ? 2 :
        Get_UNCOND_OP(PS.MEM_CS) ? 1 :
        Get_BR_OP(PS.MEM_CS) ?
        ((n && ((PS.MEM_CC >> 2) & 0X01)) || (z && ((PS.MEM_CC >> 1) & 0X01)) || (p && (PS.MEM_CC & 0X01))) ? 1 : 0 : 0 : 0;
    if(PS.MEM_V && Get_BR_OP(PS.MEM_CS) && (!n & !p & !z)){
        MEM_PCMUX = 1;
    } else if (PS.MEM_V && Get_BR_OP(PS.MEM_CS) && !( (n && ((PS.MEM_CC >> 2) & 0X01)) || (z && ((PS.MEM_CC >> 1) & 0X01)) || (p && (PS.MEM_CC & 0X01))))
        branch = 0;



    TARGET_PC = PS.MEM_ADDRESS;
    printf("TARGET PC IS 0X%04X\n", TARGET_PC);
    ...
    int we0, we1;
    int mem0 = PS.MEM_ADDRESS & 0x01;
    we0 = Get_DCACHE_RW(PS.MEM_CS) ? !mem0 ? 1 : 0 : 0;
    we1 = Get_DCACHE_RW(PS.MEM_CS) ? (Get_DATA_SIZE(PS.MEM_CS) & !mem0) || ((!Get_DATA_SIZE(PS.MEM_CS)) & mem0) ? 1 : 0 : 0;

    int write = !Get_DATA_SIZE(PS.MEM_CS) ? ((PS.MEM_ALU_RESULT & 0X0FF) << 8) | (PS.MEM_ALU_RESULT & 0X0FF) : PS.MEM_ALU_RESULT;
    int dcache_r;

    if(v_dcache_en)
        dcache_access(PS.MEM_ADDRESS, &TRAP_PC, write, &dcache_r, we0, we1);
    else
        TRAP_PC = 0;
    ...
    NEW_PS.SR_DATA = Get_DATA_SIZE(PS.MEM_CS) && ((PS.MEM_ADDRESS & 0X01) == 0) ? TRAP_PC : !Get_DATA_SIZE(PS.MEM_CS) && !(PS.MEM_ADDRESS & 0X01) ? SIGNEX(TRAP_PC & 0X0FF, 7) : SIGNEX((TRAP_PC & 0XFF00) >> 8, 7);
...
void AGEX_stage() {
    ...
    if (LD_MEM) {
        ...
        int sr2 = Get_SR2MUX(PS.AGEX_CS) ? SIGNEX(PS.AGEX_IR & 0X1F, 4) : PS.AGEX_SR2;
        int aluk = Get_ALUK(PS.AGEX_CS);
        int sr1 = PS.AGEX_SR1;

        int shifter = (PS.AGEX_IR >> 4) & 0x03;
        int shift_amt = PS.AGEX_IR & 0x0F;
        int shift = Low16bits( (shifter == 0) || (shifter == 2) ? (sr1 << shift_amt) :
                (shifter == 1) ? (sr1 >> shift_amt) : Low16bits( SIGNEX( sr1, 15 ) >> shift_amt ));

        int alu = Low16bits(!aluk ? sr1 + sr2 : aluk == 1 ? sr1 & sr2 : aluk == 2 ? sr1 ^ sr2 : sr2);

        NEW_PS.MEM_ALU_RESULT = Get_ALU_RESULTMUX(PS.AGEX_CS) ? alu : shift;

        int addr2 = (PS.AGEX_CS[AGEX_ADDR2MUX1] << 1) | PS.AGEX_CS[AGEX_ADDR2MUX0];;
        int lshf1 = PS.AGEX_CS[AGEX_LSHF1];

        NEW_PS.MEM_ADDRESS = Low16bits(!PS.AGEX_CS[AGEX_ADDRESSMUX] ? (PS.AGEX_IR & 0X0FF ) << 1 :
                (PS.AGEX_CS[AGEX_ADDR1MUX] ? PS.AGEX_SR1 : PS.AGEX_NPC) + ((!addr2 ? 0 :
                        addr2 == 1 ? SIGNEX((PS.AGEX_IR & 0X3F), 5) :
                        addr2 == 2 ? SIGNEX((PS.AGEX_IR & 0x1FF), 8) : SIGNEX((PS.AGEX_IR & 0x7FF), 10)) << lshf1 ));
        ...
        NEW_PS.MEM_IR = PS.MEM_IR;
        NEW_PS.MEM_DRID = PS.MEM_DRID;
        NEW_PS.MEM_CC = PS.MEM_CC;
        NEW_PS.MEM_NPC = PS.MEM_NPC;
        NEW_PS.MEM_ADDRESS = PS.MEM_ADDRESS;
        NEW_PS.MEM_ALU_RESULT = PS.MEM_ALU_RESULT;
        NEW_PS.MEM_V = PS.MEM_V;

        for(ii = 0; ii < NUM_MEM_CS_BITS; ii++){
            NEW_PS.MEM_CS [ii] = PS.MEM_CS[ii];
        }
    }
}
...
void DE_stage() {
    ...
    int CONTROL_STORE_ADDRESS = ((PS.DE_IR >> 11) << 1) | ((PS.DE_IR >> 5) & 0x01);  /* You need to implement the logic to set the value of this variable. Look
                                                                                        at the figure for DE stage */
    int ii, jj = 0;
    int LD_AGEX = mem_stall ? 0 : 1; /* You need to write code to compute the value of
                    LD.AGEX signal */

    /* your code for DE stage goes here */

    v_de_br_stall = PS.DE_V & CONTROL_STORE[CONTROL_STORE_ADDRESS][BR_STALL];
    int sr1 = (PS.DE_IR >> 6 ) & 0x07 ;
    int sr2 = (PS.DE_IR >> 13) & 0X01 ? (PS.DE_IR >> 9) & 0X07 : (PS.DE_IR & 0X07);
    dep_stall = PS.DE_V ?
        ( Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (( (sr1 == agex_reg_id) & v_agex_ld_reg) || ((sr1 == mem_reg_id) & v_mem_ld_reg) || ( (sr1 == sr_reg_id) & v_sr_ld_reg)) ? 1 :
          ( Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (( (sr2 == agex_reg_id) && v_agex_ld_reg) || ((sr2 == mem_reg_id) && v_mem_ld_reg) || ( (sr2 == sr_reg_id) && v_sr_ld_reg)) ? 1 :
            ( Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc)) ? 1 : 0 )) : dep_stall;
    ...
    if (LD_AGEX) {
        ...
        NEW_PS.AGEX_DRID = CONTROL_STORE[CONTROL_STORE_ADDRESS][DRMUX] ? 7 : (PS.DE_IR >> 9) & 0X07;
        NEW_PS.AGEX_NPC = PS.DE_NPC;
        NEW_PS.AGEX_CC = (N << 2) | (Z << 1) | P;
        NEW_PS.AGEX_SR1 = REGS[ (PS.DE_IR >> 6 ) & 0x07 ];
        NEW_PS.AGEX_SR2 = REGS[ (PS.DE_IR >> 13) & 0X01 ? (PS.DE_IR >> 9) & 0X07 : (PS.DE_IR & 0X07) ];
        NEW_PS.AGEX_V = PS.DE_V & !dep_stall ? 1 : 0;
        ...
        NEW_PS.AGEX_DRID = PS.AGEX_DRID;
        NEW_PS.AGEX_NPC = PS.AGEX_NPC;
        NEW_PS.AGEX_SR1 = PS.AGEX_SR1;
        NEW_PS.AGEX_SR2 = PS.AGEX_SR2;
        NEW_PS.AGEX_CC = PS.AGEX_CC;
        NEW_PS.AGEX_V = PS.AGEX_V;

        for (ii = 0; ii < NUM_AGEX_CS_BITS; ii++) {
            NEW_PS.AGEX_CS[ii] = PS.AGEX_CS[ii];
        }
    }

    REGS[ sr_reg_id ] = v_sr_ld_reg ? sr_reg_data : REGS[ sr_reg_id ] ;
    N = v_sr_ld_cc ? sr_n : N;
    Z = v_sr_ld_cc ? sr_z : Z;
    P = v_sr_ld_cc ? sr_p : P;
...
void FETCH_stage() {
    ...
    int LD_PC = icache_r & !mem_stall & !dep_stall & !v_de_br_stall & !v_agex_br_stall;
    LD_PC = v_mem_br_stall ? 1 : LD_PC;
    LD_PC = !branch ? 0 : LD_PC;
    /**************************/
    /**************************/
    /*************************/


    NEW_PS.DE_IR = LD_DE ? ir : PS.DE_IR;
    NEW_PS.DE_NPC = LD_DE ? PC + 2 : PS.DE_NPC;
    NEW_PS.DE_V = LD_DE ? LD_PC & !v_mem_br_stall : PS.DE_V; /* THIS IS NOT DONE */

    PC = LD_PC ? !MEM_PCMUX ? PC + 2 : MEM_PCMUX == 1 ? TARGET_PC : MEM_PCMUX == 2 ? TRAP_PC : PC : PC;
    branch = 1;
}
