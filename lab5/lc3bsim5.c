...
void eval_micro_sequencer() {
    ...
    /* check this */
    if(CL(next_state) != 32)
        exception = cur_state != 51 ? 0 :  ( CL(VA) & 0x01 ) || (( (CL(PSR) >> 15) & 0x01) == 1 && ((CL(MDR) >> 3) & 0x01) == 0) || !( (CL(MDR) >> 2) & 0x01);
    CL(EXCV) = exception == 1 ?
        (CL(VA) & 0x01) ? 0x03 :
        (((CL(PSR) >> 15) & 0x01) == 1 && ((CL(MDR) >> 3) & 0x01) == 0)  ? 0x04 :
        !((CL(MDR) >> 2) & 0x01) ? 0x02 : CL(EXCV) : CL(EXCV);

    if((CL(next_state) == 29 && CL(EXCV) == 0x03) || (CL(next_state) == 35 && CL(EXCV) == 0x03)){
        exception = 0;
        CL(EXCV) = 0x00;
    } else if(CL(next_state) == 32 && CL(EXCV) == 0x04){
        exception = 0;
    } else if(CL(next_state) == 28 && CL(EXCV) == 0x04){
        exception = 0;
        CL(EXCV) = 0x00;
    }
    ...
    NL(next_state) = INST(LD_MAR) & !INST(T) ? state : CL(next_state);
    state = INST(LD_MAR) & !INST(T) ? 43 : state;
    state = INST(NSMUX) == 1 ? CL(next_state) : state;
    if(state != 18 && state != 19){
        state = exception || interrupt ? 48 : state;
        NL(EX_IN_Flag) = exception & !interrupt ? 1 : CL(EX_IN_Flag);
        CL(EX_IN_Flag) = !interrupt ? NL(EX_IN_Flag) : CL(EX_IN_Flag);
    ...
    NL(EXCV) = state == 10 || state == 11 ? 0x05 : CL(EXCV);
    NL(EX_IN_Flag) = state == 10 || state == 11 ? 1 : CL(EX_IN_Flag);
}
...
void drive_bus() {
    ...
    BUS = ( (INST(GATE_PC) == 1) ? TD(GATE_PC) :
            ...
            (INST(GATE_VECT) == 1) ? TD(GATE_VECT) :
            (INST(GATE_PTBR) == 1) ? TD(GATE_PTBR) : 0);
}
...
void latch_datapath_values() {
     ...
     */
    int ir = CL(IR);
    int muxmar = (INST(MUXMAR1) << 1) | INST(MUXMAR0);
    NL(MAR) = ( INST(LD_MAR) == 1) ? muxmar == 0 ? BUS :
        muxmar == 1 ? (CL(MDR) & 0x3E00) | (CL(VA) & 0x1FF) : muxmar == 2 ? (BUS & 0xFF00) | ((CL(VA) & 0xFE00 ) >> 8) : CL(MAR) : CL(MAR);

    CL(MDR) = CL(STATE_NUMBER) == 51 ? CL(MDR) | (CL(flag) << 1) | 0x01 : CL(MDR);
    ...
    NL(VA) = INST(LD_VA) == 1 ? CL(MAR) : CL(VA);

    if( INST(MIO_EN) == 1 && INST(R_W) == 1){
        MEMORY[CL(MAR) >> 1][0] = ( (INST(DATA_SIZE) == 0 && (CL(MAR) & 0x01) == 0) || INST(DATA_SIZE) == 1 ? CL(MDR) & 0xFF : MEMORY[CL(MAR) >> 1][0]);
        MEMORY[CL(MAR) >> 1][1] = ( (INST(DATA_SIZE) == 0 && (CL(MAR) & 0x01) == 1) || INST(DATA_SIZE) == 1 ? (CL(MDR) >> 8) & 0xFF : MEMORY[CL(MAR) >> 1][1]);
    }
    ...
    NL(PTBR) = CL(PTBR);
    NL(flag) = INST(LD_FLAG) ? INST(FLAG_VALUE) : CL(flag);
}
