...
void eval_bus_drivers() {
    ...
    int ir = CL(IR);
    int sr1 = (INST( SR1MUX ) == 0 ? CL(REGS)[ (ir >> 9) & 0x07] : CL(REGS)[ (ir >> 6) & 0x07 ] );
...
void latch_datapath_values() {
    ...
    NL(REGS)[ (INST( DRMUX ) == 0 ? (ir >> 9) & 0x07 : 7 ) ] = Low16bits( (INST( LD_REG ) == 0) ? CL(REGS)[ (INST( DRMUX ) == 0 ? (ir >> 9) & 0x07 : 7 ) ] : BUS );

    int pcmux = (INST(PCMUX1) << 1) | INST(PCMUX0);
    int sr1 = (INST( SR1MUX ) == 0 ? CL(REGS)[ (ir >> 9) & 0x07] : CL(REGS)[ (ir >> 6) & 0x07 ] );
    int addr2mux = (INST(ADDR2MUX1) << 1) | INST(ADDR2MUX0);
    NL(PC) = ( (INST(LD_PC) == 1) ? ( (pcmux == 0) ? CL(PC) + 2 : ( (pcmux == 1) ? BUS : Low16bits(ADDER(sr1) ))) : CL(PC) );
